...
export function createMessagePreview(message) {
  ...
  // Extract sender name
  const senderName = message.sender?.match(/^(.+?)\s*<(.+?)>$/)?.[1]?.trim() || message.sender || 'Unknown';
...
function createActivationSection(content, contentType) {
  ...
  if (isPasswordReset) {
    const displayText = content.length > 50 ? content.substring(0, 40) + '...' : content;
    return `
