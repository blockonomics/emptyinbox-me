...
export function extractActivationCode(htmlBody, textBody, subject) {
  ...
  const patterns = [
    /activation\s+code[^A-Z0-9]*([A-Z0-9]{4,12})/g,
    /verification\s+code[^A-Z0-9]*([A-Z0-9]{4,12})/g,
    /confirm\s+code[^A-Z0-9]*([A-Z0-9]{4,12})/g,
    /your\s+code[^A-Z0-9]*([A-Z0-9]{4,12})/g,
    /code[^A-Z0-9]*([A-Z0-9]{6,12})/g,
